...
import baic.obd.protocol.model.Body;
import baic.obd.protocol.model.Head;
...
import baic.obd.protocol.util.Constants;
		...
		try {
			...
			for (String packageData : packageDataArray) {
				...
				for (String data : dataArray) {
					...
					// if (partArray[index] != null &&
					// !"".equals(partArray[index])) {
					// String binStr =
					// ParseUtils.hexString2BinaryString(partArray[index]);
					// head.setEventId(Integer.valueOf(binStr.substring(2), 2));
					// if (binStr.length() >= 8) {
					// head.setEventFlag(binStr.substring(binStr.length() - 8,
					// binStr.length() - 7));
					// }
					// }
					model.setHead(head);
