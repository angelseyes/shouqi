...
folder are mapped to the application's root namespace, 'BAICClassicApp'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "BAICClassicApp"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'BAICClassicApp' in the same way
as `"BAICClassicApp/sass/src"`.
